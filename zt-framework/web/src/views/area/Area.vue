	...
	<div class="app-container calendar-list-container">
		<div class="filter-container crud-menu clearfix">
			...
			<div class="form_search clearfix" style="padding:0 10px;">
				<el-form :inline="true" :model="listQuery" class="search-form-inline">
					<el-form-item label="输入搜索:">
						<el-input v-model="listQuery.areaName" placeholder="行政区域名称/区域编码" ></el-input>
			...
			<el-button class="float-fr" type="primary" @click="handleCreate('form')" icon="edit" size="mini" style="margin-bottom:10px;"
			 v-if="userData.permissions.includes('sys_area_add')">添加行政区域
			</el-button>
		</div>
		<div style="height:calc(100% - 120px)">
			<z-table :data="tableData" @size-change="handleSizeChange" @current-change="handleCurrentChange" :total='total'>
				<div slot="table-column">
					...
					<el-table-column align="center" label="行政区域名称">
					...
					<el-table-column align="center" label="行政区划代码">
						<template slot-scope="scope">
		...
		<div class="dialog_mask_mine">
			<el-dialog :title="dialogStatus=='create'?'添加行政区域':'编辑行政区域'" :visible.sync="dialogFormVisible" width="45%" :close-on-click-modal="false">
				<div class="fix-width">
					<el-form :model="form" class="clearfix" ref="form" :rules="rules">
						<el-row :gutter="20">
							<el-col :span="12">
								<el-form-item label="行政区域名称" :label-width="formLabelWidth" prop="areaName">
									<el-input v-model="form.areaName" autocomplete="off" :maxlength="20"></el-input>
								</el-form-item>
							</el-col>
							<el-col :span="12">
								<el-form-item label="行政区域代码" :label-width="formLabelWidth" prop="code">
...
<script>
	...
	export default {
		...
		methods: {
			...
			//搜索
			onSubmit() {
                this.listQuery.areaName = this.listQuery.areaName.trim();
                this.listQuery.areaName= this.listQuery.areaName.replace(/\\|\/|\?|\？|\*|\"|\“|\”|\'|\‘|\’|\<|\>|\{|\}|\[|\]|\【|\】|\：|\:|\、|\^|\$|\!|\~|\`|\|/g,"");
				this.getList();
			},
			//修改分页
			...
			handleUpdate(row) {
				...
				detailArea(row.areaId).then(res => {
				...
				this.$refs[form].validate((valid) => {
					if (valid) {
						if (this.dialogStatus == 'create') {
							addArea(_this.form).then((response) => {
							...
							updateArea(_this.form).then((response) => {
								if(response.data.data==false){
									_this.$message.error(response.data.msg);
								}else{
									_this.dialogFormVisible = false;
			...
			handleExport(){
				if(this.tableData.length>0){
					downloadExcel(this.listQuery,'行政区域').then(response => {});
				}else{
