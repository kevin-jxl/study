<template>
	<div class="app-container calendar-list-container">
		<div class="filter-container crud-menu clearfix">
			...
			<div class="form_search clearfix" style="padding:0 10px;">
				<el-form :inline="true" :model="listQuery" class="search-form-inline">
					<el-form-item label="设备名称:">
						<el-input v-model="listQuery.documentName" placeholder="请输入设备名称"></el-input>
					</el-form-item>
                    <el-form-item label="设备编号:">
						<el-input v-model="listQuery.documentName" placeholder="请输入设备编号"></el-input>
					</el-form-item>
                    <el-form-item label="投用时间:" style="margin-left:10px;">
                        <el-date-picker
			...
			<div class="float-fl table-title">设备列表</div>
			<el-button class="float-fr" type="primary" @click="handleCreate('form')" icon="edit" size="mini" style="margin-bottom:10px;"
			 v-if="userData.permissions.includes('sys_area_add')">添加设备
			</el-button>
		</div>
		<div style="height:calc(100% - 130px)">
			<z-table :data="tableData" @size-change="handleSizeChange" @current-change="handleCurrentChange" :total='total'>
				<div slot="table-column">
					...
					<el-table-column align="center" label="出场日期">
						<template slot-scope="scope">
							<span>{{scope.row.terminationTime}}</span>
						</template>
					</el-table-column>

					<el-table-column align="center" label="投用日期">
						<template slot-scope="scope">
							<span>{{scope.row.cooperationMode}}</span>
						</template>
					</el-table-column>
					...
					<el-table-column align="center" label="关联泊位">
						<template slot-scope="scope">
					...
					<el-table-column label="操作" align="center" width="300" v-if="userData.permissions.includes('sys_area_edit') || userData.permissions.includes('sys_area_del')">
						<template slot-scope="scope">
                            ...
                            <el-button size="mini" type="text" color="info" v-if="userData.permissions.includes('sys_area_edit')" @click="linkInterface(scope.row)">附件</el-button>
							<el-button size="mini" type="text" color="info" v-if="userData.permissions.includes('sys_area_edit')" @click="EwmVisible(scope.row)">二维码生成</el-button>
						</template>
					</el-table-column>
				</div>
			</z-table>
        ...
        <div class="dialog_mask_mine">
            <el-dialog title="二维码生成" :visible.sync="dialogEwmVisible" width="45%" :close-on-click-modal="false">
            <div class="fix-width">
                <!-- :rules="rules" -->
                <el-form :model="form" class="clearfix" ref="form" >
						<el-row :gutter="20">
							<el-col :span="12">
								<el-form-item label="设备编号" :label-width="formLabelWidth" prop="documentName">
							...
							<el-col :span="12">
								<el-form-item label="设备名称" :label-width="formLabelWidth" prop="code">
									<el-input v-model="form.code" autocomplete="off" :maxlength="40" placeholder="请输入">></el-input>
								</el-form-item>
							</el-col>
                            <el-col :span="12">
								<el-form-item label="所属车场" :label-width="formLabelWidth">
									<el-select placeholder="请选择设备类型" v-model="form.cooperationMode">
										<el-option label="请选择" value=""></el-option>
										<!-- <el-option :label="v.name" :value="v.com_id" v-for="(v,i) in syscompanyList" :key="i"></el-option> -->
									</el-select>
                            ...
                            <el-col :span="12">
								<el-form-item label="位置坐标" :label-width="formLabelWidth" prop="code">
									<el-input v-model="form.code" autocomplete="off" :maxlength="40" placeholder="请输入">></el-input>
								</el-form-item>
							</el-col>
                            <el-col :span="12">
								<el-form-item label="安装时间" :label-width="formLabelWidth" prop="leading">
                                    <el-date-picker
                                        v-model="value1"
                                        type="date"
                                        placeholder="选择日期">
                        ...
                        <el-form-item label="附件地址" :label-width="formLabelWidth" style="width: 90%;">
                            <el-input type="text" placeholder="数字不超过200字" v-model="form.remark" autocomplete="off" :maxlength="100" :rows="4"></el-input>
                        </el-form-item>
					</el-form>    
            </div>
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="handleSubmit('form')">{{dialogStatus=='create'?'确定':'保存'}}</el-button>
                <el-button @click="dialogEwmVisible = false">取 消</el-button>
            </div>
            </el-dialog>
        </div>
		<div class="dialog_mask_mine">
			<el-dialog :title="dialogStatus=='create'?'添加设备':'编辑设备'" :visible.sync="dialogFormVisible" width="45%"
				...
				<div class="fix-width">
					<el-form :model="form" class="clearfix" ref="form" :rules="rules">
						<el-row :gutter="20">
							...
							<el-col :span="12">
								<el-form-item label="生成厂家" :label-width="formLabelWidth">
									<el-input v-model="form.signingParty" autocomplete="off" :maxlength="20" placeholder="请输入">></el-input>
								</el-form-item>
							</el-col>
							<el-col :span="12">
								<el-form-item label="出场日期" :label-width="formLabelWidth" prop="leading">
                                    <el-date-picker
                                        v-model="value1"
                                        type="date"
                                        placeholder="选择日期">
                            ...
                            <el-col :span="12">
								<el-form-item label="投用日期" :label-width="formLabelWidth" prop="leading">
                                    <el-date-picker
                                        v-model="value1"
                                        type="date"
                                        placeholder="选择日期">
                            ...
                            <el-col :span="24">
								<el-form-item label="关联泊位" :label-width="formLabelWidth">
									<el-button style="color:#4768F3;font-weight:bold;" @click="addDevice()">添加</el-button>
								</el-form-item>
							</el-col>
                            <div v-for="(item,id) in deveiceList" :key="id">
                                <el-col :span="11">
                                    <el-form-item label="泊位名称" :label-width="formLabelWidth">
                                        <el-input v-model="form.names" autocomplete="off" :maxlength="40" placeholder="请输入">></el-input>
                                    </el-form-item>
                                </el-col>
                                <el-col :span="11">
                                    <el-form-item label="泊位编号" :label-width="formLabelWidth">
                                       <el-input v-model="form.code" autocomplete="off" :maxlength="40" placeholder="请输入">></el-input>
                                    </el-form-item>
                                </el-col>
                                <el-col :span="2">
                                    <el-form-item label="">
                                        <i class="el-icon-error" v-show="id!=0" @click="handleDel(deveiceList,id)"></i>
                                    </el-form-item>
                                </el-col>
                            </div>
						</el-row>
